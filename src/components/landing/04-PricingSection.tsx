...
export default function PricingSection() {
  ...
  return (
    <section id="pricing" className="bg-brand-slate py-24">
      <div className="mx-auto max-w-5xl px-6 lg:px-8">
        ...
        <div className="mt-10 mx-auto max-w-md rounded-2xl border border-brand-emerald/40 bg-brand-slate-light/50 p-8 shadow-[0_0_40px_rgba(16,185,129,0.15)]">
          ...
          <ul className="mt-6 space-y-3">
            {FREE_INCLUDES.map((item) => (
              <li key={item} className="flex items-center gap-2.5 text-sm text-slate-300">
                <Check className="h-4 w-4 shrink-0 text-brand-emerald" />
                {item}
              </li>
            ))}
          </ul>
          <Link
            href="/signup"
            className="mt-8 block rounded-full bg-brand-emerald px-6 py-3 text-center text-sm font-semibold text-brand-slate shadow-[0_0_20px_rgba(16,185,129,0.5)] transition-shadow hover:shadow-[0_0_30px_rgba(16,185,129,0.75)]"
          >
            Get Started Free
          </Link>
        </div>
